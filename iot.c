...
static void enqueue_task_iot(struct rq *rq, struct task_struct *p, int flags)
{
     ...
     */
     printk ("enqueue_task_iot\n");
     // FIXME: add the entity to the run queue
     printk(" enque_task_iot:1");  
     struct list_head *queue;

     printk(" enque_task_iot:2");  
     queue = rq->iot_rq.active.queue + p->prio;

     printk(" enque_task_iot:3");  
     list_add(&p->iot_t.run_list, queue);
    
     printk(" enque_task_iot:4");  
     // FIXME: update the bitmap of the priority array
     __set_bit(p->prio, rq->iot_rq.active.bitmap);
    
    printk(" enque_task_iot:5");  
    p->iot_t.on_list = 1;
    p->iot_t.on_rq = 1;

       
    printk(" enque_task_iot:6"); 
    int prev_prio = rq->iot_rq.highest_prio.curr;
	if (p->prio < prev_prio)
    {
        rq->iot_rq.highest_prio.curr = p->prio;
    }

    printk(" enque_task_iot:7"); 
    // FIXME: set the total number of running tasks in struct iot_rq
    rq->iot_rq.iot_nr_running += 1;
    printk(" enque_task_iot: 8");
    rq->nr_running+=1;
    printk(" enque_task_iot: exiting");  
}
...
static void dequeue_task_iot(struct rq *rq, struct task_struct *p, int flags)
{
	...
	if (iot_se->on_list)
    {
	    ...
	    if (list_empty(rq->iot_rq.active.queue + p->prio))
        {
            int x = p->prio;
             __clear_bit(x, rq->iot_rq.active.bitmap);
        }
...
static struct task_struct *pick_next_task_iot(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
    ...
    if (idx < MAX_IOT_PRIO)
    {
        printk("pick_next_task_iot: if-block");
    ...
    {
        printk("pick_next_task_iot: else-block");
        return NULL;
    }
